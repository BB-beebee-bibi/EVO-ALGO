...
'''
    ...
    def evaluate_fitness(self, code: str) -> Tuple[float, Dict[str, float]]:
        ...
        """
        # Syntax check before evaluation
        try:
            compile(code, '<string>', 'exec')
        except SyntaxError as e:
            print(f"Syntax error in evolved code: {e}")
            return 0.0, {'functionality': 0.0, 'efficiency': 0.0, 'robustness': 0.0, 'maintainability': 0.0}
        ...
        try:
            ...
            if 'organize_files' not in local_vars:
                return 0.0, {'functionality': 0.0, 'efficiency': 0.0, 'robustness': 0.0, 'maintainability': 0.0}
            ...
            functionality_score = self._evaluate_functionality(result, test_files)
            
            # Evaluate efficiency (placeholder)
            efficiency_score = 0.8
            
            # Evaluate robustness
            robustness_score = self._evaluate_robustness(code)
    ...
    def _evaluate_functionality(self, result: Dict[str, List[str]], test_files: Dict[str, str]) -> float:
        """Evaluate how well the code organizes files."""
        if not result:
            return 0.0
        ...
        for category, files in result.items():
            for file_path in files:
                ...
                if filename in test_files:
                    expected_category = self._get_category_from_extension(test_files[filename])
                    if category == expected_category:
                        correct_moves += 1
        
        return correct_moves / total_files if total_files > 0 else 0.0
    ...
    def get_allowed_imports(self) -> List[str]:
        ...
        return self.get_required_boundaries()['allowed_imports']
