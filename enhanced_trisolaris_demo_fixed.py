...
class EnhancedTrisolarisDemonstration:
    ...
    def run_demonstration(self):
        ...
        try:
            ...
            # Patch the engine's evaluate_population method to fix the can_proceed issue
            original_evaluate_population = engine.evaluate_population
            
            def patched_evaluate_population():
                # Check if resource scheduler indicates we should throttle
                if engine.resource_monitor and hasattr(engine.resource_monitor, 'should_throttle') and engine.resource_monitor.should_throttle():
                    logger.warning("Resource constraints exceeded. Throttling evaluation.")
                    engine.fitness_scores = [s if i < len(engine.fitness_scores) else 0
            ...
            # Apply the patch
            engine.evaluate_population = patched_evaluate_population
