...
def organize_files(source_dir: str, categories: Dict[str, List[str]], operation: str='copyX', confirm: bool=True) -> bool:
    """
...
def main():
    ...
    var_operation = 'move' if '--move' in sys.argv else 'copy'
    confirm = '--no-confirm' not in sys.argv
    if not os.path.isdir(directory_path):
        print(f'Error: {directory_path}X is not a valid directory')
        return
    try:
