...
class TaskTemplateLoader:
    ...
    def get_fallback_template(self, task_type: str) -> Optional[str]:
        """Get a fallback template when the regular one isn't found."""
        if task_type == "bluetooth_scan":
            return """
import bluetooth
import time
    ...
    def parse_request(self, request: str) -> Dict[str, Any]:
        ...
        if "update" in request_lower and "second" in request_lower:
            try:
                ...
                self.logger.warning(f"Failed to parse update interval: {e}")
        ...
        # Add description from the request
        ...
        try:
            ...
            for gen in range(1, settings["gens"] + 1):
                ...
                try:
                    ...
                    # Apply adaptive tweaking using proper integration
                    try:
                        # Use record_metrics if available, otherwise fall back to update_parameters
                        ...
                        if 'mutation_rate' in new_params and engine.mutation_rate != new_params["mutation_rate"]:
                            # Check if print_color is available in adaptive_tweaker
                            ...
                            try:
                                ...
                                # Fall back if not properly imported in adaptive_tweaker
                                print(f"Adjusting mutation rate: {engine.mutation_rate:.3f} -> {new_params['mutation_rate']:.3f}")
            ...
            self.logger.error(f"Critical error during evolution: {str(e)}")
            self.logger.error(traceback.format_exc())
