"""
GPT LLM client implementation.

This module provides a concrete implementation of the LLM client interface
for OpenAI's GPT models, handling API communication, response parsing,
and error handling specific to GPT.
"""
...
class GPTClient(LLMClient):
    ...
    This class implements the LLMClient interface for OpenAI's GPT models,
    providing methods to evaluate code ethics using GPT's capabilities.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        ...
        Args:
            api_key: OpenAI API key (defaults to config if not provided)
            model: GPT model to use (defaults to config if not provided)
        """
        ...
        if not api_key:
            logger.warning("No OpenAI API key provided. Set OPENAI_API_KEY in environment or config.")
    ...
    async def evaluate_ethics(self, code: str) -> Dict[str, Any]:
        """
        Evaluate the ethics of code using GPT.
        ...
        try:
            ...
            payload = {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": "You are an expert in ethical code evaluation."},
            ...
            logger.info("Sending ethics evaluation request to GPT")
            ...
            # Find JSON in the response (GPT might wrap it in markdown code blocks)
            json_start = content.find("{")
            json_end = content.rfind("}") + 1
            
            if json_start == -1 or json_end == 0:
                raise LLMClientError("Could not find JSON in GPT response")
            ...
            error_msg = f"HTTP error during GPT ethics evaluation: {str(e)}"
            ...
            error_msg = f"Error during GPT ethics evaluation: {str(e)}"
            logger.error(error_msg)
            raise LLMClientError(error_msg)
    
    async def evaluate_gurbani_alignment(self, code: str, ethics_report: Dict[str, Any]) -> Dict[str, Any]:
        """
        Evaluate how well code aligns with Gurbani principles using GPT.
        ...
        try:
            ...
            payload = {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": "You are an expert in evaluating code against Gurbani principles."},
                    {"role": "user", "content": prompt}
                ],
                "temperature": 0.1,  # Low temperature for more deterministic responses
                "max_tokens": 4000
            ...
            # Make the API request
            logger.info("Sending Gurbani alignment evaluation request to GPT")
            response = await self.client.post(
                self.api_url,
                headers=self.headers,
                json=payload
            ...
            error_msg = f"HTTP error during GPT Gurbani evaluation: {str(e)}"
            logger.error(error_msg)
            raise LLMClientError(error_msg)
        except json.JSONDecodeError as e:
            error_msg = f"Failed to parse GPT response as JSON: {str(e)}"
            ...
            error_msg = f"Error during GPT Gurbani evaluation: {str(e)}"
            logger.error(error_msg)
            raise LLMClientError(error_msg)
    
    async def get_improvement_suggestions(self, code: str, ethics_report: Dict[str, Any]) -> List[str]:
        """
        Get suggestions for improving code ethics using GPT.
        ...
        try:
            ...
            payload = {
                "model": self.model,
                "messages": [
                    {"role": "system", "content": "You are an expert in ethical code improvement."},
                    {"role": "user", "content": prompt}
                ],
            ...
            # Make the API request
            logger.info("Sending improvement suggestions request to GPT")
            response = await self.client.post(
                self.api_url,
                headers=self.headers,
                json=payload
            ...
            if json_start == -1 or json_end == 0:
                ...
                logger.warning("Could not find JSON array in GPT response, extracting suggestions from text")
                lines = content.split("\n")
            ...
            if not isinstance(suggestions, list):
                raise LLMClientError("GPT response is not a list of suggestions")
            
            logger.info(f"Generated {len(suggestions)} improvement suggestions")
            return suggestions
            
        except httpx.HTTPError as e:
            error_msg = f"HTTP error during GPT suggestions generation: {str(e)}"
            logger.error(error_msg)
            raise LLMClientError(error_msg)
        except json.JSONDecodeError as e:
            error_msg = f"Failed to parse GPT response as JSON: {str(e)}"
            ...
            error_msg = f"Error during GPT suggestions generation: {str(e)}"
            logger.error(error_msg)
            raise LLMClientError(error_msg)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.error(f"GPT health check failed: {str(e)}")
            return False